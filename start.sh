...
if [ "$LAN_IP" != "localhost" ]; then
  echo -e "  ${GREEN}┌─────────────────────────────────────────────────────┐${RESET}"
  echo -e "  ${GREEN}│  👑 你是 HOST — 其他人按以下方式加入：              │${RESET}"
  echo -e "  ${GREEN}│                                                     │${RESET}"
  echo -e "  ${GREEN}│  1. 其他人【不需要】运行 start.sh                   │${RESET}"
  echo -e "  ${GREEN}│  2. 用浏览器直接打开：${BOLD}${WEB_ACCESS_URL}${RESET}${GREEN}  │${RESET}"
  echo -e "  ${GREEN}│  3. 输入相同的 Channel ID 即可协作                  │${RESET}"
  echo -e "  ${GREEN}└─────────────────────────────────────────────────────┘${RESET}"
else
